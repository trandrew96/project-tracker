...
export default class CreateComment extends Component {
  ...
  onSubmit(e) {
    ...
    if (this.state.comment.trim() !== "") {
      axios
        .post("http://localhost:5000/api/tasks/submit-comment", commentInfo)
        .then((res) => {
          this.setState({
    ...
    if (
      ...
      this.state.assignee !== this.props.assignee
    ) {
      axios
        .post(
          "http://localhost:5000/api/tasks/update/" + this.props.taskId,
          newTaskData
        )
        .then((res) => {
          console.log(res);
          ...
          <Col md={3}>
            <FormGroup>
              ...
              <Input
                ...
                className="form-control"
              >
                <option key="no one" value="(no one)">
                  (no one)
                </option>
